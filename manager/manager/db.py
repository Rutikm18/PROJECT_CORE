...
-- Per-agent API keys (generated by agent on first run, sent during enrollment).
-- The manager never issues keys — it only records them.
...
class Database:
    ...
    async def get_agent_key(self, agent_id: str) -> str | None:
        """Return the stored 64-hex-char API key for agent_id, or None."""
        async with aiosqlite.connect(self.path) as db:
            async with db.execute(
                "SELECT api_key_hex FROM agent_keys WHERE agent_id=?", (agent_id,)
            ) as cur:
                row = await cur.fetchone()
                return row[0] if row else None

    async def upsert_agent_key(
        self, agent_id: str, api_key_hex: str, enrolled_ip: str = ""
    ) -> None:
        """Store or update a per-agent API key (enrollment / key rotation)."""
        now = int(time.time())
        async with aiosqlite.connect(self.path) as db:
            await db.execute("""
                INSERT INTO agent_keys(agent_id, api_key_hex, enrolled_at, enrollment_ip)
                VALUES(?,?,?,?)
                ON CONFLICT(agent_id) DO UPDATE SET
                    api_key_hex   = excluded.api_key_hex,
                    enrolled_at   = excluded.enrolled_at,
                    enrollment_ip = excluded.enrollment_ip
            """, (agent_id, api_key_hex, now, enrolled_ip))
            await db.commit()
