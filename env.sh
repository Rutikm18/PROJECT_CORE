...
# ── Main ──────────────────────────────────────────────────────────────────────
banner
# Tracks whether an .env already existed on entry, so the final summary can prove
# (per secret) that a re-run preserved rather than regenerated it. Initialised
# here because `set -u` is on and load_existing only sets it when .env is present.
ENV_EXISTED=0
load_existing
...
# The contract: a secret already present in .env (loaded above) is NEVER
# regenerated — re-running env.sh only ever fills in the ones that are missing.
# Each secret records whether this run created it (*_IS_NEW) so the final summary
# can report "preserved (unchanged)" vs "NEWLY GENERATED" and prove nothing was
# revoked. This mirrors the existing JWT_SECRET handling below.
step "Generating secrets"

ADMIN_TOKEN_IS_NEW=0
if [[ -z "${ADMIN_TOKEN:-}" ]]; then
  ADMIN_TOKEN_IS_NEW=1
  ADMIN_TOKEN="$(gen_token 'sk-admin-' 24)"
fi

ENROLLMENT_TOKENS_IS_NEW=0
if [[ -z "${ENROLLMENT_TOKENS:-}" ]]; then
  ENROLLMENT_TOKENS_IS_NEW=1
...
if [[ "$ADMIN_TOKEN_IS_NEW" == "1" ]]; then
  ok "Admin token   : generated — ${ADMIN_TOKEN:0:16}... (full value in .env)"
else
  info "Admin token   : preserved from existing .env (unchanged)"
fi
if [[ "$OPEN_ENROLLMENT" == "false" ]]; then
  if [[ "$ENROLLMENT_TOKENS_IS_NEW" == "1" ]]; then
    ok "Enroll token  : generated — ${ENROLLMENT_TOKENS:0:16}... (full value in .env)"
  else
    info "Enroll token  : preserved from existing .env (unchanged)"
  fi
fi
...
# Write to a temporary file first, then atomically move it into place (below).
# A direct 'cat > .env' truncates the live file *before* the heredoc is evaluated,
# so any expansion failure inside it — e.g. a stray '$10' in a comment, which
# `set -u` treats as unbound positional $1 — would wipe .env, destroying
# ADMIN_TOKEN/JWT_SECRET and forcing every secret to regenerate on the next run.
# Staging through ENV_TMP means a failed write leaves the real .env untouched.
ENV_TMP=".env.tmp.$$"
...
# .env holds ADMIN_TOKEN, JWT_SECRET and any API keys — keep it owner-only. Set
# the mode on the temp file, then atomically move it into place: the live .env
# changes only here, on full success, so a failed write can never wipe it.
chmod 600 "$ENV_TMP" 2>/dev/null || warn "Could not chmod 600 ${ENV_TMP} — check permissions"
mv "$ENV_TMP" .env
...
# ── Re-run safety report ───────────────────────────────────────────────────────
# Only meaningful on a re-run (an .env already existed on entry). Proves, per
# secret, that this run preserved rather than regenerated it — so re-running
# env.sh to change TLS/network settings never logs you out, never invalidates a
# running agent's token, and never makes the stored AI key undecryptable.
if [[ "$ENV_EXISTED" == "1" ]]; then
  _cred_status() {
    if [[ "$2" == "1" ]]; then
      echo -e "    ${YELLOW}$1 : NEWLY GENERATED (was missing from .env)${NC}"
    else
      echo -e "    ${GREEN}$1 : preserved (unchanged)${NC}"
    fi
  }
  echo -e "${BOLD}  Re-run safety — existing secrets were NOT revoked:${NC}"
  _cred_status "ADMIN_TOKEN            " "$ADMIN_TOKEN_IS_NEW"
  _cred_status "ENROLLMENT_TOKENS      " "$ENROLLMENT_TOKENS_IS_NEW"
  _cred_status "JWT_SECRET             " "$JWT_SECRET_IS_NEW"
  _cred_status "DASHBOARD_PASSWORD_HASH" "$DASHBOARD_PASSWORD_IS_NEW"
  info "A secret already present in .env is never regenerated — only missing ones are created."
  echo ""
fi
