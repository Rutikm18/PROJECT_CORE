...
EOF
  ...
  cat > "${JARVIS_DIR}/Caddyfile" <<EOF
# =============================================================
#  Jarvis Manager — Caddy reverse proxy
#  auto-generated by aws_setup.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
# =============================================================
...
:80 {
    redir https://{host}:${BIND_PORT}{uri} permanent
}
...
EOF
  ...
  ok "Caddyfile written for HTTPS port ${BIND_PORT}"
}
...
smoke_test() {
  ...
  local PUBLIC_URL="https://localhost:${BIND_PORT}/health"
  local INTERNAL_URL="http://localhost:8080/health"
  local RESPONSE
  local RETRIES=5

  for i in $(seq 1 $RETRIES); do
    RESPONSE=$(curl -fsk "$PUBLIC_URL" 2>/dev/null || true)
    if [[ -n "$RESPONSE" ]]; then
      ok "Public Caddy health endpoint responded: ${RESPONSE}"
      return
    fi
    RESPONSE=$(sudo docker compose exec -T manager \
      curl -fs "$INTERNAL_URL" 2>/dev/null || true)
    if [[ -n "$RESPONSE" ]]; then
      ok "Internal FastAPI health endpoint responded: ${RESPONSE}"
      warn "Caddy HTTPS endpoint is not ready yet; check: sudo docker compose logs caddy"
      return
    fi
    sleep 3
  done
  warn "Health endpoint did not respond within ${RETRIES} retries."
  warn "Manager may still be starting. Check: sudo docker compose logs manager caddy"
}
