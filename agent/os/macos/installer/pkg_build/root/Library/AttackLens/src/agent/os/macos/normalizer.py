...
def _norm_metrics(raw: Any) -> dict:
    ...
    # New canonical psutil path (cpu_percent / mem_percent from volatile.py)
    if "cpu_percent" in raw:
        return {
            "cpu_percent":      _f(raw.get("cpu_percent")),
            "mem_percent":      _f(raw.get("mem_percent")),
            "cpu_per_core":     raw.get("cpu_per_core") or [],
            "cpu_cores":        _i_opt(raw.get("cpu_cores")),
            "cpu_cores_physical": _i_opt(raw.get("cpu_cores_physical")),
            "cpu_freq_mhz":     _f_opt(raw.get("cpu_freq_mhz")),
            "mem_used_mb":      _i(raw.get("mem_used_mb")),
            "mem_total_mb":     _i(raw.get("mem_total_mb")),
            "mem_available_mb": _i_opt(raw.get("mem_available_mb")),
            "swap_percent":     _f_opt(raw.get("swap_percent")),
            "swap_used_mb":     _i_opt(raw.get("swap_used_mb")),
            "swap_total_mb":    _i_opt(raw.get("swap_total_mb")),
            "disk_read_mb_s":   _f_opt(raw.get("disk_read_mb_s")),
            "disk_write_mb_s":  _f_opt(raw.get("disk_write_mb_s")),
            "net_sent_mb_s":    _f_opt(raw.get("net_sent_mb_s")),
            "net_recv_mb_s":    _f_opt(raw.get("net_recv_mb_s")),
    ...
    # Legacy psutil path (old field names — backward-compat for stored data)
    if "cpu_pct" in raw:
        return {
            "cpu_percent":    _f(raw.get("cpu_pct")),
            "mem_percent":    _f(raw.get("mem_pct")),
            "cpu_per_core":   [],
            "cpu_cores":      _i_opt(raw.get("cpu_cores")),
            ...
            "swap_percent":   _f_opt(raw.get("swap_pct")),
            "swap_used_mb":   _i_opt(raw.get("swap_used_mb")),
            "swap_total_mb":  _i_opt(raw.get("swap_total_mb")),
            "disk_read_mb_s": None, "disk_write_mb_s": None,
            "net_sent_mb_s":  None, "net_recv_mb_s":  None,
            "load_1m":        _f_opt(raw.get("load_1m")),
            "load_5m":        _f_opt(raw.get("load_5m")),
            "load_15m":       _f_opt(raw.get("load_15m")),
            "uptime_sec":     _i_opt(raw.get("uptime_sec")),
    ...
    # CLI text path: parse raw strings from top/sysctl/vm_stat
    cpu   = _parse_cpu_line(raw.get("cpu_raw", raw.get("cpu", "")))
    load  = _parse_loadavg(raw.get("load_raw", raw.get("load", "")))
    vm    = _parse_vm_stat(raw.get("vmstat_raw", raw.get("vmstat", "")))
    swap  = _parse_swap(raw.get("swap_raw", raw.get("swap", "")))
    ps    = vm["page_size"]
    ...
    return {
        ...
        "mem_available_mb": None,
        "swap_percent":   round(swap["used_mb"] / swap["total_mb"] * 100, 1) if swap["total_mb"] > 0 else None,
