...
# ── Validate ──────────────────────────────────────────────────────────────────
if (-not (Test-Path ".\attacklens-agent.exe")) {
    Write-Error "attacklens-agent.exe not found in current directory."
}
if (-not (Test-Path ".\attacklens-watchdog.exe")) {
    Write-Error "attacklens-watchdog.exe not found in current directory."
}
...
Copy-Item ".\attacklens-agent.exe"   -Destination $BinDir -Force
Copy-Item ".\attacklens-watchdog.exe" -Destination $BinDir -Force
# Restrict write access on binaries (read + execute only for service account)
icacls "$BinDir\attacklens-agent.exe"    /inheritance:r `
    /grant:r "NT AUTHORITY\SYSTEM:(RX)" `
    /grant:r "BUILTIN\Administrators:(RX)" 2>&1 | Out-Null
icacls "$BinDir\attacklens-watchdog.exe" /inheritance:r `
    /grant:r "NT AUTHORITY\SYSTEM:(RX)" `
    /grant:r "BUILTIN\Administrators:(RX)" 2>&1 | Out-Null
Write-Host " done" -ForegroundColor Green
...
pid_file     = "$($InstallDir -replace '\\', '\\')\\attacklens-agent.pid"

[binaries]
agent    = "$($BinDir -replace '\\', '\\')\\attacklens-agent.exe"
watchdog = "$($BinDir -replace '\\', '\\')\\attacklens-watchdog.exe"
...
& sc.exe create MacIntelAgent `
    binpath= "`"$BinDir\attacklens-agent.exe`"" `
...
& sc.exe create MacIntelWatchdog `
    binpath= "`"$BinDir\attacklens-watchdog.exe`"" `
